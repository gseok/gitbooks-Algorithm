...
public static void updateLazy(int nodeId, int nodeL, int nodeR) {
    // 업데이트할 부분이 있으면
    if (lazy[nodeId] != 0) {
        // 한번에 tree에다가 lazy 업데이트 해야 하는 부분을 실제 업데이트 합니다.
        tree[nodeId] += (nodeR - nodeL + 1) * lazy[nodeId];
...
public static int query(int nodeId, int nodeL, int nodeR, int L, int R) {
    // query할때, 아직 업데이트 안한게 있을 수 있으니, 업데이트 하고 값을 달라고 하면 됩니다.
    updateLazy(nodeId, nodeL, nodeR);
    if (L > nodeR || R < nodeL) {
        return 0;
    }
    ...
    int mid = (nodeL + nodeR) / 2;
    return query(nodeId * 2, nodeL, mid, L, R) + query(nodeId * 2 + 1, mid + 1, nodeR, L, R);
}

public static void updateRange(int nodeId, int nodeL, int nodeR, int L, int R, int diff) {
    ...
    if (L <= nodeL && R >= nodeR) {
        tree[nodeId] += (nodeR - nodeL + 1) * diff;
        if (nodeL != nodeR) {
            lazy[nodeId * 2] += diff;
            lazy[nodeId * 2 + 1] += diff;
        }
        return;
    }

    int mid = (nodeL + nodeR) / 2;
    updateRange(nodeId * 2, nodeL, mid, L, R, diff);
    updateRange(nodeId * 2 + 1, mid + 1, nodeR, L, R, diff);
